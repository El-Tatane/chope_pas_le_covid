...
def plot_triple_sir(list_t, list_predict_S, list_predict_I, list_predict_R, list_true_S=None, list_true_I=None, list_true_R=None):
    ...
    for (list_pred, list_true), tilte in zip([[list_predict_S, list_true_S], [list_predict_I, list_true_I], [list_predict_R, list_true_R]], list_label):
            ...
            if list_true is not None:
                ax.plot(list_t[:len(list_true)], list_true/1000, 'r', alpha=0.5, lw=2, label='True')
            ax.set_xlabel('Time (days)')
            ax.set_ylabel('Person Number (10^3)')
            ax.yaxis.set_tick_params(length=0)
            ax.xaxis.set_tick_params(length=0)
...
def plotseird(t, S, E, I, R, D=None, L=None, R0=None, Alpha=None, CFR=None, day_bar=None, real_I=None):
    ...
    if real_I is not None:
        ax.plot(t[:len(real_I)], real_I, 'k', alpha=0.7, linewidth=2, label='real infected')
    if day_bar is not None:
        ax.axvline(day_bar, label="01 march")
    if D is not None:
